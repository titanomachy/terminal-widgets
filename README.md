...
TerminalWidgets works on Linux, macOS, and Windows.
...
```mermaid
flowchart TB
  ecosystem["Nim Terminal Ecosystem"]

  ecosystem --> styling["Styling & Text<br/>Formatting<br/>────────────<br/>TerminalStyle<br/>(ANSI, Colors,<br/>TrueColor)"]
  ecosystem --> data["Tabular & Visual<br/>Data<br/>────────────<br/>TerminalTable<br/>TerminalGraph<br/>(Charts, Spark)"]
  ecosystem --> layout["Structural &<br/>Layout<br/>────────────<br/>TerminalLayout<br/>(Trees, Panels,<br/>Callouts)"]
  ecosystem --> status["Status &<br/>Progress<br/>────────────<br/>TerminalStatus<br/>(Spinners,<br/>Progress Bars)"]
  ecosystem --> interaction["Interactive UI<br/>Components<br/>────────────<br/>TerminalPrompt<br/>TerminalWidgets<br/>(Prompts, Tabs)"]

  layout --> terminalScreen["TerminalScreen<br/>Alternate Screen · Raw Mode · Cursor<br/>Control · Terminal Geometry"]
```

## Contents

- [TerminalWidgets](#terminalwidgets)
...
| --- | --- |
| [`checkbox.nim`](examples/checkbox.nim) | Boolean checkbox controls |
| [`switch.nim`](examples/switch.nim) | Boolean switch controls |
| [`radio_group.nim`](examples/radio_group.nim) | Exclusive-choice radio controls |
| [`scroll_list.nim`](examples/scroll_list.nim) | Keyed list navigation |
| [`menu.nim`](examples/menu.nim) | Keyed menu navigation and explicit activation events |
| [`tabs.nim`](examples/tabs.nim) | Active-page layout |
| [`text_field.nim`](examples/text_field.nim) | Text editing |
| [`headless_form.nim`](examples/headless_form.nim) | A finite mixed-widget form used by compatibility checks without requiring a TTY |
| [`form.nim`](examples/form.nim) | A guarded interactive form where Enter submits and exits, Escape exits, and Ctrl+C cancels |
| [`core_model.nim`](examples/core_model.nim) | Constructing and updating a retained preferences tree using only the facade |
| [`composition_focus.nim`](examples/composition_focus.nim) | Fixed/flex layout, stored allocations, and focus traversal without terminal I/O |
| [`basic_controls.nim`](examples/basic_controls.nim) | Driving checkbox, switch, and radio state through the public dispatcher and printing final values |
| [`selection_navigation.nim`](examples/selection_navigation.nim) | Shared list/menu navigation and explicit menu activation |
| [`tabs_retained.nim`](examples/tabs_retained.nim) | Preserved state, semantic headers, and tree-managed page removal and reordering across three retained pages |
| [`text_field_editing.nim`](examples/text_field_editing.nim) | Unicode-safe insertion/deletion, scalar limits, placeholder text, and validator/submission events without opening a terminal |
| [`validated_form.nim`](examples/validated_form.nim) | Two validated fields, failed and successful submissions, and focus movement using only the public facade |
| [`render_frame.nim`](examples/render_frame.nim) | Laying out a small form, rendering an exact-size plain frame, and reporting optional cursor metadata |
| [`companion_output.nim`](examples/companion_output.nim) | Placing static graph-like string output in a frame without adding a companion dependency |
| [`interactive_form.nim`](examples/interactive_form.nim) | Running a small validated form through the owned TerminalScreen lifecycle in a real terminal |
| [`runtime_demo.nim`](examples/runtime_demo.nim) | The finite injected runtime sequence used to regenerate the README animation deterministically |
| [`api_overview_demo.nim`](examples/api_overview_demo.nim) | The finite `controls`, `tabs`, `text`, `events`, `layout`, and `frames` sequences used by the API overview animations |
| [`package_import.nim`](examples/package_import.nim) | Verifying that a facade import does not initialize a terminal session |
